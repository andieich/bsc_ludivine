...
```{r}
dat_chl_count_after <- left_join(dat_chl_after_persample,
                        dat_counts_after_persample, 
                        by = "sample_id")
```
...
plot_chl_after_persample<- dat_chl_count_after %>% 
  ggplot(aes(x = site, col = size,))+
  geom_point(aes( y = dat_chl_count_after), 
             position = position_jitterdodge(jitter.width = .2, 
                                             dodge.width = .6),
             alpha = .6)+
  geom_errorbar(data = dat_chl_count_after, 
                aes(x = site,
                  ymin = dat_chl_count_after - se,
                  ymax = dat_chl_count_after + se),
                position = position_dodge(width = .6),
                width = .3,
                show.legend = F)+
  geom_point(data = dat_chl_count_after, 
             aes(x = site, 
                 y = dat_chl_count_afteri), 
             size = 4,
             position = position_dodge(width = .6), 
             shape = 21, 
             fill = "white", 
             ...
             show.legend = F)+
  labs(x = "Site", 
       y = expression("Chlorophyll per"~cm^2), 
       title = "Chlorophyll per Symbiodiniaceae")+
  scale_colour_manual(values = col_size, name = "Size")+
  theme_minimal()+
  theme(plot.title = element_text(hjust = 0.5))
plot_chl_after_persample
